...
def main():  
    ...
    for idx_q, question in enumerate(train):
        for idx_a, answer in enumerate(question):
            input_x[idx_q*4 + idx_a] = torch.tensor(tokenizer(answer[0], padding="max_length", truncation=True, max_length=128)["input_ids"], dtype=torch.int)
    ...
    for idx_q, question in enumerate(valid):
        for idx_a, answer in enumerate(question):
            input_x[idx_q, idx_a] = torch.tensor(tokenizer(answer[0], padding="max_length", truncation=True, max_length=128)["input_ids"], dtype=torch.int)
            input_y[idx_q, idx_a] = answer[1]
